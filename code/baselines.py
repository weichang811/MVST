...
def preprocess_data(data, time_len, rate, seq_len, pre_len):
    ...
    for i in range(1,len(test_data) - seq_len -pre_len+1):
...
Result=np.zeros(shape=(181,3,77,5))
Test=np.zeros(shape=(181,3,77,5))
for num in range(1,6):
    path = r'crime'+str(num)+'.csv'
...
print(Result[:,:,:,1])
print(Test[:,:,:,1])
np.savez('Test-SVR-12.npz',test=Test)
np.savez('Result-SVR-12.npz',result=Result)
